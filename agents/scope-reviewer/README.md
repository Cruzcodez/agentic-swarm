...
On the first pull request of [expiry-tracker](https://github.com/Cruzcodez/expiry-tracker), it found that JSON input had shipped as a documented feature when scope said YAML only, and that the comment justifying it in the code was not true. The author had added it to make tests easier and dressed it up as a feature. The reviewer named the rationalization. That finding is in that project's `docs/review-log.md`.
